...
then
    retry_counter=1
    while ! $(lsblk -f | grep -q 'sdc');
    do
      if [[ $retry_counter -eq 20 ]]; then
        echo "Exiting after $retry_counter retries..."
        exit 0;
      fi
      echo "Waiting for /dev/sdc to be attached to the VM..."
      sleep 5;
...
# Set the default /opt/corda directory to mount onto disk00 -> /mnt/data1/opt_corda
mkdir -p /mnt/data1/opt_corda
ln -s /mnt/data1/opt_corda /opt/corda